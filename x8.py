...
def gui():
    ...
    Label(root, text="Select multiple files by holding Ctrl (Cmd on Mac)", font=('Arial', 10, 'italic')).pack(pady=15)
    ...
    aes_key_paths = StringVar()
    multiple_aes_keys = StringVar()
    ...
    def select_encrypted_files():
        paths = filedialog.askopenfilenames(filetypes=[("Encrypted files", "*.aes *.rsa")])
        if paths:
    ...
    def batch_decrypt_files(encrypted_file_paths, aes_key_paths=None, private_key_path=None, public_key_path=None):
        ...
        if not aes_keys and not private_key_path:
            messagebox.showerror("Error", "No keys were selected for decryption. Please select AES keys for AES files or a private key for RSA files.")
        ...
        # Check if private key path exists for RSA
        if private_key_path and not os.path.exists(private_key_path):
            messagebox.showerror("Error", f"The RSA private key file at {private_key_path} does not exist. Please check your selection.")
            update_queue.put(("Error", f"The RSA private key file at {private_key_path} does not exist."))
            return
    ...
    # GUI elements
    plaintext_image_frame = ttk.Frame(root, padding="10")
    plaintext_image_frame.pack()
    Label(plaintext_image_frame, text="Select Image File (for encryption):").pack(side=tkinter.LEFT)
    Button(plaintext_image_frame, text="Browse", command=select_files).pack(side=tkinter.LEFT)
    Entry(root, textvariable=file_path, width=50).pack()

    ciphertext_image_frame = ttk.Frame(root, padding="10")
    ciphertext_image_frame.pack()
    Label(ciphertext_image_frame, text="Select Encrypted File (for decryption):").pack(side=tkinter.LEFT)
    Button(ciphertext_image_frame, text="Browse", command=select_encrypted_files).pack(side=tkinter.LEFT)
    Entry(root, textvariable=encrypted_file_path, width=50).pack()

    encryption_method_frame = ttk.Frame(root, padding="10")
    ...
    Checkbutton(root, text="Use Existing Keys", variable=use_existing_keys).pack(pady=5)
    ...
    aes_key_frame = ttk.Frame(root, padding="10")
    aes_key_frame.pack()
    Label(aes_key_frame, text="Select AES Key (for AES decryption):").pack(side=tkinter.LEFT, pady=5)
    Button(aes_key_frame, text="Browse", command=lambda: select_aes_key(aes_key_path)).pack(side=tkinter.LEFT)
    Entry(root, textvariable=aes_key_path, width=50).pack()

    Checkbutton(root, text="Use Certificate", variable=use_certificate).pack(pady=5)

    Button(root, text="Select Existing Certificate", command=select_cert).pack()
    Entry(root, textvariable=cert_path, width=50).pack()

    # Frame for certificate type selection
    cert_type_frame = ttk.Frame(root, padding="10")
    cert_type_frame.pack(pady=10)

    Button(cert_type_frame, text="Generate Certificate", command=generate_certificate).pack(side=tkinter.LEFT, padx=5)
    Label(cert_type_frame, text="Type:").pack(side=tkinter.LEFT)
    cert_type_menu = OptionMenu(cert_type_frame, cert_type, 'self-signed', 'Trusted CA')
    cert_type_menu.pack(side=tkinter.LEFT)

    # Placeholder frame for domain name (ensures space is reserved)
    domain_placeholder = ttk.Frame(root, height=50, width=500)  # Reserve enough height for the domain frame
    domain_placeholder.pack_propagate(False)  # Prevent the placeholder from resizing
    domain_placeholder.pack(pady=5)

    # Domain Name field, positioned after Certificate Type
    domain_name_frame = ttk.Frame(domain_placeholder, padding="10", borderwidth=2, relief="groove")

    domain_name_label = Label(domain_name_frame, text="Domain Name (for Trusted CA):")
    domain_name_label.pack(side=tkinter.LEFT)
    domain_name_entry = Entry(domain_name_frame, textvariable=domain_name, width=50)
    domain_name_entry.pack(side=tkinter.LEFT, padx=5)
    ...
    def toggle_domain_name(*args):
        if cert_type.get() == "Trusted CA":
            ...
            # This reserves the space by packing with the same options but makes it invisible
    ...
    buttons_frame = ttk.Frame(root, padding="10", borderwidth=2, relief="groove")
    ...
    bottom_frame = ttk.Frame(root, borderwidth=2, relief="groove")
    bottom_frame.pack(side=tkinter.BOTTOM, padx=5, pady=5)
    ...
    Button(left_buttons_frame, text="Generate RSA Keys", command=generate_keys).pack(side=tkinter.LEFT, padx=5)
